...
public class TryFindPeerAndDownload implements DownloadTorrent, DHTTask {
	...
	public void subTask_findpeers(byte[] hash, LocalDHTNode localDHTNode) {
		...
		String hashString = DHTUtils.byteArrayToHexString(hash);
		//如果已有当前记录
		if(respository.exist(hashString)) {
			return;
		}

		// 如果提交哈希正在执行下载,防止连续提交
		if (findPeersHash.containsKey(hashString)) {
			return;
		}
		// 队列任务太多咱就不提交了
...
//				allNodesQueue.add(key);
	...
	class RunTask implements Runnable {
		...
		public void run() {
			...
			try {
				...
				//这里使用栈，在findpeers逻辑中，最后找到的节点下载的概率相对要高一些
	...
	public synchronized void start() {
		...
		//
		respository = LuceneTorrentRespository.getInstance();
